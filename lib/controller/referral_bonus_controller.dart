...
import 'package:hyip_pro/data/model/response_model/referral_bonus_search_model.dart';
...
class ReferralBonusController extends GetxController{
  ...
  double get totalReferralAmount {
    double totalAmount = 0.0;

    // Iterate through referralBonusItems and sum up the amounts
    for (var item in referralBonusItems) {
      // Extract numeric part of the string (assuming "10.0 USD" format)
      String numericPart = item.amount.replaceAll(RegExp(r'[^0-9.]'), '');

      // Ensure that the numeric part is not empty before parsing
  ...
  ReferralBonusSearchModel referralBonusSearchModel = ReferralBonusSearchModel();
  ...
  Future<dynamic> getReferralBonusSearchData(
    ...
    update();
    ApiResponse apiResponse = await referralBonusSearchRepo.getReferralBonusSearchData(searchUser,createdAt,page: page);
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      ...
      if (apiResponse.response!.data != null) {
        ...
        if(apiResponse.response!.data["message"]=="Email Verification Required"){
          ...
          referralBonusSearchModel = ReferralBonusSearchModel.fromJson(apiResponse.response!.data!);
